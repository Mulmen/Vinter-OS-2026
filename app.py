...
def find_file_safely(filename: str) -> Path | None:
    """
    Letar efter filen i typiska repo-strukturer:
    - ./data/<filename>
    - ./<any>/data/<filename>
    - ./<filename> (fallback)
    """
    ...
    # Lägg till första matchen i alla undermappar: */data/<filename>
    candidates.extend(list(cwd.glob(f"*/data/{filename}")))
    # Och som sista utväg: sök rekursivt (begränsa för att vara snabb)
    # (rglob kan vara tungt, men repo är oftast liten)
    candidates.extend(list(cwd.rglob(f"data/{filename}")))
...
def load_athletes() -> pd.DataFrame:
    athletes_path = find_file_safely("athletes.csv")

    if athletes_path is None:
        # Debug-info så du ser exakt var Streamlit kör ifrån
        st.error("Saknar athletes.csv. Appen hittade ingen fil i någon data/-mapp.")
        st.write("**Current working directory:**", str(Path.cwd()))
        st.write("**Exempel på vad appen letar efter:** `data/athletes.csv` (i samma mappnivå som app.py)")
        st.stop()
    ...
    st.caption(f"Läser atleter från: `{athletes_path}`")
...
with tabs[2]:
    ...
    if not ok:
        ...
        for _, row in res.iterrows():
            ...
            with c5:
                new_medal = st.selectbox("Medalj", MEDALS, index=MEDALS.index(row["medal"]) if row["medal"] in MEDALS else 0, key=f"admin_{row['athlete_id']}")
...
with tabs[3]:
    st.subheader("Backup")
    st.download_button("Ladda ner picks.json", data=PICKS_JSON.read_bytes() if PICKS_JSON.exists() else b"{}", file_name="picks.json", mime="application/json", use_container_width=True)
    st.download_button("Ladda ner results.csv", data=RESULTS_CSV.read_bytes() if RESULTS_CSV.exists() else b"athlete_id,medal\n", file_name="results.csv", mime="text/csv", use_container_width=True)
